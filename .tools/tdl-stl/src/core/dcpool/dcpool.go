...
func (p *pool) invoker(ctx context.Context, dc int) tg.Invoker {
	...
	if dc == p.current() { // can't transfer dc to current dc
		...
		// Trial: establish the cross-DC session on the regular endpoint first,
		// then reuse its in-memory session for dedicated media connections.
		var bootstrap telegram.CloseInvoker
		bootstrap, err = p.api.DC(ctx, dc, 1)
		if err == nil {
			invoker, err = p.api.MediaOnly(ctx, dc, p.size)
			_ = bootstrap.Close()
		}
	}
