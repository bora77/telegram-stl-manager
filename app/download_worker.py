...
from app.telegram_cli import TelegramCLI, CLIError as WorkerError, STATE, CLI_STATE, safe_filename, completed_transfer_receipt
from app.release_rules import release_month, in_scope, existing_release_directory
from app.download_plan import DownloadPlan
from app.transfer_metrics import TransferMeter
from app.file_delivery import deliver, mount_identity, digest, release_lock
from app.release_images import extract_images, ExtractionError, MissingVolumeError, volume_key, flat_image_name, listing, is_image_attachment, image_warnings
...
class Worker:
    ...
    def select_uploads(self,sub,entries):
        ...
        from app.release_images import is_archive
        groups={};excluded=set()
        for entry in entries:
            item=entry['item']
    ...
    def execute_adaptive(self):
        from app.adaptive_downloads import AdaptiveDownloads
