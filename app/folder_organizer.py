...
from app.release_images import is_image_attachment, volume_key, image_warnings
from app.release_rules import release_month, existing_release_directory
from app.subscription_store import SubscriptionStore
from app.telegram_cli import TelegramCLI
...
class Organizer:
    ...
    def status(self):
        ...
        if plan.get('state') not in ('starting','scanning') and plan.get('backend')=='cli':
            from app.organizer_versions import project
        ...
        from app.organizer_apply import application_status, RUNNING
    ...
    def start(self, payload):
        ...
        with (self.root / 'data/operation.lock').open('a') as operation:
            ...
            with (self.root / 'data/organizer.log').open('ab') as log:
                subprocess.Popen([sys.executable, '-m', 'app.folder_organizer', plan['id']], cwd=self.root,
