...
def application_status(store):
    ...
    from app.organizer_repair import candidates
...
def ready_groups(plan):
    from app.organizer_versions import choices
...
def start_application(store, payload, resume=False, repair=False):
    ...
    with (store.root / 'data/operation.lock').open('a') as operation:
        ...
        if resume:
            ...
            job = json.loads(path.read_text())
            from app.organizer_versions import upgrade
            if upgrade(job,plan):
            ...
            if repair:
                from app.organizer_repair import prepare
        ...
        with (store.root / 'data/organizer.log').open('ab') as log:
            Popen([sys.executable, '-m','app.organizer_apply', job['id']], cwd=store.root,
            ...
            for index,group in enumerate(self.job['groups']):
                ...
                try:
                    with release_lock(self.store.root,self.job['base'],self.job['creator_folder'],group['month'],self.stopped,waiting):
                        ...
                        if group.get('version'):
                            from app.organizer_versions import apply
                            ...
                            if group.get('repairs'):
                                from app.organizer_repair import run
                        ...
                        self.save()
                        if group.get('repairs'):
                            from app.organizer_repair import cleanup
                            cleanup(self,group)
                        if group.get('version'):
                            from app.organizer_versions import cleanup
                            cleanup(self,group)
                    ...
                    if group.get('version'):
                        from app.organizer_versions import failed
