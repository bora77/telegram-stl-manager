...
class RunManager:
    ...
    def start(self,payload):
        with self.lock,(self.root/'data/operation.lock').open('a') as operation:
            ...
            with (self.root/'data/worker.log').open('ab') as log:
                self.process=subprocess.Popen([sys.executable,'-m','app.download_worker',batch],cwd=self.root,stdout=log,stderr=log,start_new_session=True)
            return run
    def resume(self,payload):
        with self.lock,(self.root/'data/operation.lock').open('a') as operation:
            ...
            try:
                with (self.root/'data/worker.log').open('ab') as log:
                    self.process=subprocess.Popen([sys.executable,'-m','app.download_worker',run['id']],cwd=self.root,stdout=log,stderr=log,start_new_session=True)
    ...
    def prepare_redownload(self,run,rows):
        from app.download_plan import DownloadPlan
        from app.telegram_cli import STATE, CLI_STATE
