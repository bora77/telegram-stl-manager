...
from collage_layout import COLORS, FORMATS, LAYOUTS, geometry, render
        ...
        with (self.cache/'operation.lock').open('a') as operation:
            ...
            try:
                with (self.cache/'worker.log').open('ab') as log:
                    subprocess.Popen([sys.executable,str(self.root/'collages.py'),'export',export_id],cwd=self.root,stdout=log,stderr=log,start_new_session=True)
            except OSError:
...
if __name__=='__main__':
    ...
    store=CollageStore(SubscriptionStore(Path(__file__).resolve().parent))
