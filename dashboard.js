...
async function refreshQueue(){
  try{
    ...
    bandwidthPanel.hidden=!(bandwidth?.active_files>0);
    if(!bandwidthPanel.hidden){const rate=bandwidth.speed_bps,usage=rate==null?null:100*rate/(bandwidth.target_mbps*1e6);document.getElementById('bandwidth-status').textContent=`Combined download: ${speed(rate)} / ${bandwidth.target_mbps} MB/s target${usage==null?'':` · ${Math.round(usage)}% utilized`}`;progress(document.getElementById('bandwidth-progress'),rate??0,rate==null?null:bandwidth.target_mbps*1e6)}
