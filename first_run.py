...
from source_scope import SourceScope,load_source
from telegram_cli import CLI_STATE
    ...
    def configure_source(self,payload):
        ...
        with self.lock,self.idle():
            ...
            with (CLI_STATE/'application.lock').open('a') as guard:
                ...
                subprocess.run([sys.executable,str(self.root/'build-catalog.py')],check=True,stdout=subprocess.DEVNULL)
        return self.state()
