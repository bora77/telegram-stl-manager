...
from app.availability import Availability, INTERVAL, eligible
...
class AvailabilityTests(unittest.TestCase):
    ...
    def test_check_only_metadata_and_completed_or_ignored_disappear(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch('app.availability.TelegramCLI',return_value=client),patch('app.availability.time.time',return_value=1000):checker.check()
    ...
    def test_schedule_survives_reload_and_multiple_tabs_do_not_duplicate_check(self):
        with tempfile.TemporaryDirectory() as temp:
            store,checker=self.setup(Path(temp))
            with patch('app.availability.threading.Thread') as thread:
            ...
            with patch('app.availability.time.time',return_value=1001),patch('app.availability.threading.Thread') as thread:
                checker.start();thread.assert_not_called()
            store.queue.return_value['active']=True
            with patch('app.availability.time.time',return_value=20000),patch('app.availability.threading.Thread') as thread:
    ...
    def test_failed_scan_does_not_report_all_clear_and_changed_scope_invalidates(self):
        with tempfile.TemporaryDirectory() as temp:
            store,checker=self.setup(Path(temp))
            with patch('app.availability.TelegramCLI',side_effect=RuntimeError('offline')):checker.check()
    ...
    def test_detected_run_uses_exact_snapshot_without_scanning_other_subscriptions(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch('app.run_manager.subprocess.Popen'),patch('app.availability.TelegramCLI',side_effect=AssertionError('Must not scan')):
            ...
            with patch('app.run_manager.subprocess.Popen'):
