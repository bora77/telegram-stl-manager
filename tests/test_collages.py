...
from app.collage_layout import FORMATS, LAYOUTS, geometry, render
from app.collages import CollageError, CollageStore, collage_filename
...
class CollageTests(unittest.TestCase):
    ...
    def start(self,preview):
        with patch('app.collages.subprocess.Popen') as spawn:job=self.collages.start_export({'preview_id':preview['id']})
    ...
    def test_delivery_retry_reuses_verified_stage_and_does_not_overwrite(self):
        ...
        with patch('app.collages.deliver',side_effect=OSError('Share unavailable')):self.collages.run_export(job['id'])
        self.assertEqual(self.collages.export_status(job['id'])['state'],'failed')
        stage=self.collages.cache/(job['id']+'-output.jpg');self.assertTrue(stage.exists())
        with patch('app.collages.subprocess.Popen'):retry=self.collages.start_export({'retry_id':job['id']})
        with patch('app.collages.render',side_effect=AssertionError('Must reuse rendered output')):self.collages.run_export(job['id'])
    ...
    def test_failed_replacement_restores_previous_collage_and_retry_keeps_backup(self):
        ...
        with patch('app.collages.subprocess.Popen'):self.collages.start_export({'retry_id':job['id']})
        with patch('app.collages.render',side_effect=AssertionError('Reuse staged output')):self.collages.run_export(job['id'])
        self.assertEqual(self.collages.export_status(job['id'])['state'],'completed')
    ...
    def test_interruption_during_replacement_recovers_on_explicit_retry(self):
        original=self.start(self.collages.preview(self.payload));self.collages.run_export(original['id']);old,name=self.collages.result(original['id'])
        job=self.start(self.collages.preview({**self.payload,'theme':'light'}))
        with patch('app.collages.deliver',side_effect=KeyboardInterrupt),self.assertRaises(KeyboardInterrupt):self.collages.run_export(job['id'])
    ...
    def test_interruption_after_new_file_arrives_can_complete_the_same_save(self):
        ...
        with patch('app.collages.deliver',side_effect=delivered_then_interrupted),self.assertRaises(KeyboardInterrupt):self.collages.run_export(job['id'])
