...
import zipfile
from app.mmf_repack import repack, archive_name, VOLUME_BYTES
from app.release_images import listing, command, ExtractionError
...
class RepackTests(unittest.TestCase):
    ...
    def test_names_small_archive_and_receipt(self):
        ...
        with patch('app.mmf_repack.command',side_effect=AssertionError('should reuse verified outputs')):
    ...
    def test_compression_change_rebuilds_cached_archive(self):
        ...
        with patch('app.mmf_repack.command',wraps=command) as commands:
        ...
        with patch('app.mmf_repack.command',side_effect=AssertionError('cached output must be reused')):
    ...
    def test_metadata_removed_and_real_content_kept(self):
        ...
        with zipfile.ZipFile(source,'w') as archive:
            from tests.test_mmf_pdf import pdf, BODY
        ...
        with patch('app.mmf_repack.command',side_effect=ExtractionError('rebuild required')):
