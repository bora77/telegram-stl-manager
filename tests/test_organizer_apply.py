...
from app.folder_organizer import inventory, match_files, Organizer
from app.organizer_apply import ApplyWorker, PinnedFolder, ready_groups, start_application, check_parts
from app.release_images import flat_image_name
from app.file_delivery import digest
from app.subscription_store import SubscriptionStore
from tests.test_release_images import damaged_image_zip
...
class OrganizerApplyTests(unittest.TestCase):
    ...
    def test_apply_ignores_images_in_older_preview_and_still_extracts_archives(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            from app.organizer_apply import identity
    ...
    def request_repair(self, store, job_id):
        with patch('app.organizer_apply.Popen'):start_application(store,{'job_id':job_id},resume=True,repair=True)
    ...
    def test_manual_repair_downloads_exact_part_backs_up_original_and_finishes_release(self):
        for missing in (False,True):
            with self.subTest(missing=missing),tempfile.TemporaryDirectory() as temporary:
                ...
                with patch('app.organizer_repair.TelegramCLI',return_value=client):
                    with patch('app.organizer_apply.Popen'):start_application(store,{'job_id':job_id},resume=True)
    ...
    def test_repair_delivery_failure_reuses_download_and_preserves_local_backup(self):
        for after_publish in (False,True):
            with self.subTest(after_publish=after_publish),tempfile.TemporaryDirectory() as temporary:
                ...
                from app.file_delivery import deliver
                def fail_delivery(*args,**kwargs):
                    if after_publish:deliver(*args,**kwargs)
                    raise OSError('test interrupted delivery')
                with patch('app.organizer_repair.TelegramCLI',return_value=client):
                    with patch('app.organizer_repair.deliver',side_effect=fail_delivery):ApplyWorker(store,job_id).execute()
    ...
    def test_repair_refuses_changed_original_or_wrong_sized_download(self):
        for failure in ('original','download'):
            with self.subTest(failure=failure),tempfile.TemporaryDirectory() as temporary:
                ...
                with patch('app.organizer_repair.TelegramCLI',return_value=client):ApplyWorker(store,job_id).execute()
    ...
    def test_release_failures_continue_and_retry_preserves_completed_work_and_images(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch('app.organizer_apply.extract_images',wraps=extract_images) as extraction:
                ApplyWorker(store,job_id).execute()
    ...
    def test_cancel_and_mount_loss_still_stop_the_whole_operation(self):
        for mode in ('stop','mount'):
            with self.subTest(mode=mode),tempfile.TemporaryDirectory() as temporary:
                ...
                with patch('app.organizer_apply.extract_images',side_effect=fail) as extraction:worker.execute()
    ...
    def start(self, store, images=True):
        with patch('app.organizer_apply.Popen'):
    ...
    def test_new_preview_reuses_extraction_even_after_images_move_away(self):
        for empty,warnings in ((False,[]),(True,[]),(False,['Damaged member skipped'])):
            with self.subTest(empty=empty,warnings=warnings),tempfile.TemporaryDirectory() as temporary:
                ...
                with patch('app.organizer_apply.extract_images',side_effect=extract):ApplyWorker(store,self.start(store)).execute()
                ...
                with patch('app.organizer_apply.extract_images',side_effect=AssertionError('must skip saved extraction')) as extraction,\
                     patch('app.organizer_apply.deliver',side_effect=AssertionError('must not repeat image delivery')):
    ...
    def test_images_saved_before_archive_move_survive_a_new_job(self):
        for failure in ('images','archives'):
            with self.subTest(failure=failure),tempfile.TemporaryDirectory() as temporary:
                ...
                self.preview(store,folder);job_id=self.start(store)
                target='app.organizer_apply.deliver' if failure=='images' else 'app.organizer_apply.PinnedFolder.move'
    ...
    def test_legacy_manifests_are_migrated_and_reused(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch('app.organizer_apply.extract_images',side_effect=AssertionError('reuse legacy manifest')) as extraction:
                ApplyWorker(store,self.start(store)).execute()
    ...
    def test_resume_after_rename_and_failed_history_commit_uses_saved_images(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch('app.organizer_apply.extract_images',side_effect=AssertionError('must reuse saved extraction')):
                ApplyWorker(store,job_id).execute()
    ...
    def test_active_download_allows_apply_but_replaced_preview_is_rejected(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch('app.organizer_apply.Popen') as launch:
    ...
    def test_preview_can_start_during_download_and_rejects_second_preview(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (root/'data/worker.lock').open('a') as lock,patch('app.folder_organizer.subprocess') as process_module:
    ...
    def test_legacy_parts_resume_preserves_counts_images_history_and_cached_work(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            def extract(first,work,progress,stopped,**kwargs):
                ...
                return [target]
            with patch('app.organizer_apply.extract_images',side_effect=extract) as extraction,patch.object(store.history,'import_organized',side_effect=OSError('temporary history error')):
                ApplyWorker(store,job_id).execute()
            self.assertEqual(extraction.call_count,1)
            ...
            with patch('app.organizer_apply.extract_images',side_effect=AssertionError('must reuse both cached extractions')):
                ApplyWorker(store,job_id).execute()
            done=self.job(store)
    ...
    def test_resumed_group_rechecks_completed_part_before_touching_pending_files(self):
        for change in ('missing','changed'):
            with self.subTest(change=change),tempfile.TemporaryDirectory() as temp:
                ...
                with patch('app.organizer_apply.Popen'):
                    start_application(store,{'job_id':job_id},resume=True)
                with patch('app.organizer_apply.extract_images') as extraction:ApplyWorker(store,job_id).execute()
                extraction.assert_not_called()
                self.assertEqual(self.job(store)['state'],'completed');self.assertGreater(self.job(store)['releases_review'],0)
...
class OrganizerRemountTests(unittest.TestCase):
 def test_remount(self):
  import copy
  from app.organizer_apply import identity, revalidate_reconnected_mount
  with tempfile.TemporaryDirectory() as d:
   ...
   with patch('app.organizer_apply.mount_identity',return_value=tuple(current)):
