...
from app.organizer_apply import ApplyWorker, ready_groups, start_application
from app.organizer_versions import choices, upgrade
...
class ArchiveVersionTests(unittest.TestCase):
    ...
    def test_retry_after_publish_keeps_backups_and_reuses_extracted_images(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch('app.organizer_apply.extract_images',side_effect=AssertionError('must reuse images')):
                ApplyWorker(store,job_id).execute()
    ...
    def test_downloads_preferred_set_reuses_receipt_and_resumes_after_publish_failure(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def interrupted(*args,**kwargs):
                ...
                return result
            with patch('app.organizer_versions.TelegramCLI',return_value=client):
                with patch('app.organizer_versions.deliver',side_effect=interrupted):ApplyWorker(store,job_id).execute()
