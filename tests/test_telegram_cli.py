...
from app.telegram_cli import TelegramCLI, CLIError, ExportProgress, parse_export as parse_source_export, validate_servers
...
class ReleaseCollageGateTests(unittest.TestCase):
    def test_missing_and_invalid_collage_block_before_telegram(self):
        from app.telegram_cli import require_release_collage
...
class ServiceProcessTests(unittest.TestCase):
    @unittest.skipUnless((Path(__file__).resolve().parent.parent/'.tools/tdl-stl/tdl').is_file(), 'Build the CLI to test its local proxy')
    def test_built_cli_forwards_flags_without_opening_login_storage(self):
        binary=Path(__file__).resolve().parent.parent/'.tools/tdl-stl/tdl'
...
class CLIParseTests(unittest.TestCase):
    def test_image_document_metadata_is_kept_for_organizer_but_identifiable_for_download_filter(self):
        from app.release_images import is_image_attachment
    ...
    def test_server_choice_round_trip_does_not_start_a_download(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch('app.run_manager.subprocess.Popen') as launch:
...
class CLIProcessTests(unittest.TestCase):
        ...
        for kind in ('speed_retest_pending','speed_retest_queued'):
            with self.subTest(kind=kind),tempfile.TemporaryDirectory() as temporary:
                ...
                def run(args,work,stopped,tick,**kwargs):
                    ...
                    with patch('app.telegram_cli.time.monotonic',return_value=clock):tick()
                    with events.open('a') as stream:stream.write(json.dumps({'event':kind,'threshold_bps':20e6,'low_seconds':120})+'\n')
                    with patch('app.telegram_cli.time.monotonic',return_value=clock+1):tick()
                    with patch('app.telegram_cli.time.monotonic',return_value=clock+201):tick() # Active limit is 300s, idle is 180s.
                    ...
                    with patch('app.telegram_cli.time.monotonic',return_value=clock+202):tick()
                with patch.object(client,'_run',side_effect=run):
    ...
    def test_mid_file_server_switch_preserves_progress_and_active_monitoring(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def run(args,work,stopped,tick,**kwargs):
                ...
                with patch('app.telegram_cli.time.monotonic',return_value=clock+201):tick()
                output.write_bytes(b'0123456789')
    ...
    def test_release_server_comparison_is_once_per_month_artist_and_dc(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.object(client,'_run',side_effect=run),patch('app.telegram_cli.network_identity',return_value=('test',False)):
...
""")
            ...
            with patch.object(client,'_run',side_effect=AssertionError('must reuse verified completion')), \
                 patch('app.telegram_cli.shutil.disk_usage',return_value=type('Usage',(),{'free':0})()):
...
""")
            ...
            def delayed(args,work,stopped,tick,timeout,waiting):
                with patch('app.telegram_cli.time.monotonic',return_value=later):
                    waiting()
