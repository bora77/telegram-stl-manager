...
from app.telegram_ui import TelegramUI, UIError
...
class FocusTests(unittest.TestCase):
    ...
    def test_qt_menu_with_no_active_window_accepts_telegram_input_focus(self):
        ...
        with patch('app.telegram_ui.subprocess.check_output',return_value='WM_CLASS(STRING) = "Telegram", "TelegramDesktop"'):
            ui.guard()
    ...
    def test_other_app_focus_is_not_overridden_by_telegram_active_window(self):
        ...
        with patch('app.telegram_ui.subprocess.check_output',return_value='WM_CLASS(STRING) = "terminal", "Terminal"'),self.assertRaises(UIError):ui.guard()
        ui.xdo.assert_called_once_with('getwindowfocus')
    ...
    def test_classless_focus_can_fall_back_to_active_telegram(self):
        ...
        with patch('app.telegram_ui.subprocess.check_output',side_effect=['WM_CLASS: not found.','WM_CLASS(STRING) = "Telegram", "TelegramDesktop"']):ui.guard()
        self.assertEqual(ui.xdo.call_count,2)

class ResetTests(unittest.TestCase):
    def setUp(self):
        self.enterContext(patch('app.telegram_ui.load_source', return_value=TEST_SOURCE))
