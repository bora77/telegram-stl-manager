...
from app.release_images import ExtractionError, MissingVolumeError, extract_images
from app.folder_organizer import Organizer, inventory, match_files
from app.organizer_apply import ApplyWorker, start_application
...
class WorkerImageTests(unittest.TestCase):
    ...
    def test_review_resume_reuses_verified_files_and_clears_previous_attempt_warnings(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch('app.run_manager.subprocess.Popen'):store.runs.resume({'run_id':worker.batch})
    ...
    def test_organizer_and_download_work_simultaneously_on_different_months(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            class CLI:
                ...
                def close(self):pass
            with patch('app.organizer_apply.extract_images',side_effect=held_extraction),patch.object(download_worker,'TelegramCLI',CLI),ThreadPoolExecutor(2) as pool:
    ...
    def test_same_month_rechecks_history_when_organizer_finishes_first(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            class CLI:
                ...
                def close(self):pass
            with patch('app.organizer_apply.extract_images',side_effect=held_extraction),patch.object(download_worker,'TelegramCLI',CLI),patch.object(worker,'update',side_effect=observed_update),ThreadPoolExecutor(2) as pool:
                applying=pool.submit(organizer.execute)
                try:
    ...
    def test_same_month_reuses_identical_destination_when_download_finishes_first(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch.object(download_worker,'TelegramCLI',CLI),patch.object(organizer,'progress',side_effect=observed_progress),\
                 patch('app.organizer_apply.extract_images',side_effect=AssertionError('download already extracted images')),ThreadPoolExecutor(2) as pool:
